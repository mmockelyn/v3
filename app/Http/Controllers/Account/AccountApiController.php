...
class AccountApiController extends BaseController
{
    ...
    public function loadLatestActivity()
    {
        ...
        <div class="kt-notification">
            ...
            <?php else: ?>
                <?php foreach (auth()->user()->unreadNotifications as $activity): ?>
                    <a href="#" class="kt-notification__item">
                        <div class="kt-notification__item-icon">
                            <i class="<?= $activity->data['icon']; ?>"></i>
                        </div>
                        <div class="kt-notification__item-details">
                            ...
                            <div class="kt-notification__item-time">
                                <?php if($activity->read_at != null): ?>
                                    <?= $activity->read_at->diffForHumans() ?>
                                <?php endif; ?>
                            </div>
                        </div>
                    </a>
    ...
    public function loadLatestInvoice()
    {
        ...
        <?php foreach ($invoices as $invoice): ?>
            <tr>
                ...
                <td><a href="<?= route('Account.Invoice.show', $invoice->id) ?>" class="kt-font-success kt-font-bold"><i
                            class="la la-download"></i> </a></td>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
        <tr>
    ...
    public function delete()
    {
        try {
            ...
            Log::info("Suppression du profil !", [
                "user" => auth()->user()->name
            ]);
    ...
    public function addMethodPayment(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                Log::info("Ajout d'un mode de paiement !", [
                    "user" => auth()->user()->name,
                    "card" => [
                        "number" => Str::substr($request->number, 12, 4),
                        "brand" => $card_brand
                    ]
                ]);
                ...
                <?php foreach ($datas as $data): ?>
                    <?php if ($data->state == 0): ?>
                        ...
                        <span class="kt-timeline-v3__item-text">
                            ...
                            <a href="<?= route('Front.Blog.show', $data->blog->slug) ?>" class="kt-link kt-link--dark kt-timeline-v3__itek-link">
                                <?= $data->blog->title; ?>
                                | Posté le <?= $data->blog->published_at->format('d/m/Y à H:i') ?>
                            </a>
                        </span>
                            </div>
                        </div>
                    ...
                    <?php if ($data->published == 0): ?>
                        ...
                        <span class="kt-timeline-v3__item-text">
                            <?= $data->content; ?>
                                ...
                                <br>
                                <span class="kt-timeline-v3__item-user-name">
                            <a href="<?= route('Front.Tutoriel.show', [$data->tutoriel->subcategory->id, $data->tutoriel_id]) ?>" class="kt-link kt-link--dark kt-timeline-v3__itek-link">
                                <?= $data->tutoriel->title; ?>
                                | Posté le <?= $data->tutoriel->published_at->format('d/m/Y à H:i') ?>
                            </a>
                        </span>
                            </div>
                        </div>
            ...
            <?php else: ?>
                <?php foreach ($datas as $data): ?>
                    <tr>
                        ...
                        <td>
                            <button id="btnViewInvoice" data-id="<?= $data->id; ?>"
                                    class="btn btn-sm btn-primary btn-icon"><i class="la la-eye"></i></button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <?php else: ?>
        <?php foreach ($pms as $pm): ?>
            <tr <?php if (Carbon::createFromTimestamp(strtotime('01-' . $pm->card->exp_month . '-' . $pm->card->exp_year)) <= now()): ?> class="table-danger" <?php endif; ?>>
                <td>
                    <div class="row">
                        <div class="col-md-1"><i class="<?= AccountHelper::typeCardIcon($pm->card->brand) ?> la-3x"></i>
                        </div>
                        <div class="col-md-11">
                ...
                <td>
                    <?php if (count($pms) >= 2): ?>
                        <button id="btnDeleteMethod" data-id="<?= $pm->id; ?>" class="btn btn-sm btn-danger btn-icon"><i
        ...
        try {
            ...
            try {
                ...
                <tr <?php if (Carbon::createFromTimestamp(strtotime('01-' . $pm->card->exp_month . '-' . $pm->card->exp_year)) <= now()): ?> class="table-danger" <?php endif; ?>>
                    <td>
                        <div class="row">
                            <div class="col-md-1"><i
                                    class="<?= AccountHelper::typeCardIcon($pm->card->brand) ?> la-3x"></i></div>
                            <div class="col-md-11">
                                <strong>XXXX XXXX XXXX <?= $pm->card->last4; ?></strong><br>
                                <i>Expiration: <?= $pm->card->exp_month; ?>/<?= $pm->card->exp_year; ?></i>
                            </div>
                    ...
                    <td>
                        <?php if (count($pms) >= 2): ?>
                            <button id="btnDeleteMethod" data-id="<?= $pm->id; ?>"
                                    class="btn btn-sm btn-danger btn-icon"><i class="la la-trash-o"></i></button>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php
    ...
    public function deletePayment($pm_id)
    {
        ...
        try {
            ...
            try {
                ...
                Log::info("Suppression d'un mode de paiement !", [
                    "user" => auth()->user()->name,
                ]);
    ...
    public function isPremium()
    {
        if(auth()->user()->premium->premium == 1){
            ...
            return $this->sendResponse('false', "OK");
        }
    }
    ...
    private function registerInvoice($latest_invoice_id)
    {
        ...
        try {
            ...
            Log::info("Création d'une facture !", [
                "user" => auth()->user()->name,
                "invoice" => [
                    "number" => $in->number,
                    "amount" => $in->total
