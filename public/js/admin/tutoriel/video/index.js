  ...
  table = $("#listeTutoriel").KTDatatable({
    ...
    columns: [{
      ...
      template: function template(row) {
        return "<img src=".concat(row.img, " width=\"120\" class=\"img-fluid\"/>");
      }
    }, {
      field: 'category',
      title: "Catégorie",
      ...
      template: function template(row) {
        ...
        if (row.published === 1) {
          return "<span class=\"kt-badge kt-badge--inline kt-badge--pill kt-badge--success\"><i class=\"la la-check-circle\"></i> Publier</span><br>Publier ".concat(row.published_at);
        }

        if (row.published === 2) {
          return "<span class=\"kt-badge kt-badge--inline kt-badge--pill kt-badge--warning\"><i class=\"la la-clock-o\"></i> En attente</span><br>Publier ".concat(row.published_at);
        }
      }
    }, {
      field: 'Actions',
      ...
      template: function template(row) {
        if (row.published === 0) {
          return "\n                    <a href=\"/administrator/tutoriel/video/".concat(row.id, "\" class=\"btn btn-icon btn-sm btn-default\"><i class=\"la la-eye\"></i> </a>\n                    <a href=\"/administrator/tutoriel/video/").concat(row.id, "/edit\" class=\"btn btn-icon btn-sm btn-info\"><i class=\"la la-edit\"></i> </a>\n                    <a href=\"/administrator/tutoriel/video/").concat(row.id, "/delete\" class=\"btn btn-icon btn-sm btn-danger\"><i class=\"la la-trash-o\"></i> </a>\n                    ");
        } else {
          return "\n                    <a href=\"/administrator/tutoriel/video/".concat(row.id, "\" class=\"btn btn-icon btn-sm btn-default\"><i class=\"la la-eye\"></i> </a>\n                    ");
        }
      }
...
function loadSubcategory() {
  var category_field = $("#category_id");
  var subfield = $("#subcategory_field");
  category_field.on('change', function (e) {
    e.preventDefault();
    KTApp.block($(".modal"));
    $.get('/api/admin/tutoriel/subcategory/' + category_field.val() + '/list').done(function (data) {
      KTApp.unblock($(".modal"));
...
/***/ 40:
/*!**********************************************************!*\
  !*** multi ./resources/js/admin/tutoriel/video/index.js ***!
  \**********************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
